...
class AWD_LSTM(nn.Layer):
    ...
    def __init__(
        ...
        self.encoder = nn.Embedding(vocab_sz, emb_sz, padding_idx=pad_token) #pad_token
        self.encoder_dp = EmbeddingDropout(self.encoder, embed_p)
        self.rnns = nn.LayerList(
            [self._one_rnn(emb_sz if l == 0 else n_hid, (n_hid if l != n_layers - 1 else emb_sz) // self.n_dir,
